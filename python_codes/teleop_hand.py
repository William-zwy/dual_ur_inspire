...
class TCPClient:
    ...
    def send_pose(self, left_pose, right_pose):
        """发送左右手腕位姿：left_pose;right_pose"""
        if self.socket is None:
            print("Not connected to server")
            return False

        try:
            left = left_pose.tolist() if hasattr(left_pose, 'tolist') else left_pose
            right = right_pose.tolist() if hasattr(right_pose, 'tolist') else right_pose
            
            left_str = ",".join(f"{v:.6f}" for v in left)
            right_str = ",".join(f"{v:.6f}" for v in right)
            message = f"{left_str};{right_str}\n"

            print("=== 发送 Pose ===")
            print(f"左手位姿: {left_str}")
            print(f"右手位姿: {right_str}")
    ...
    def send_qpos(self, left_qpos, right_qpos):
        """发送左右手指关节位置：left_qpos;right_qpos"""
        if self.socket is None:
            print("Not connected to server")
            return False

        try:
            left_str = ",".join(f"{v:.6f}" for v in left_qpos)
            right_str = ",".join(f"{v:.6f}" for v in right_qpos)
            message = f"{left_str};{right_str}\n"

            print("=== 发送 Qpos ===")
            print(f"左手关节: {left_str}")
            print(f"右手关节: {right_str}")

            self.socket.sendall(message.encode())
            return True

        except Exception as e:
            print(f"Failed to send qpos: {e}")
            self.socket = None
            return False
...
if __name__ == '__main__':
    ...
    simulator = Sim()
    pose_tcp_client = TCPClient(port=12345)
    qpos_rcp_clinet = TCPClient(port=23456)

    try:
        while True:
            head_rmat, left_pose, right_pose, left_qpos, right_qpos = teleoperator.step()
            pose_tcp_client.send_pose(left_pose, right_pose)
            qpos_rcp_clinet.send_qpos(left_qpos, right_qpos)
